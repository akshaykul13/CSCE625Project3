...
import entities.HousesState;


public class HousesProblemMRV {
	...
	public static void main(String[] args) {
		...
		System.out.println("Number of states checked = " + numberOfStatesChecked);
	}
	...
	private static int[][] findSolution(int[][] solution) {
		Stack<HousesState> solutionStack = new Stack<HousesState>();
		for (int i=0; i<5; i++) {
			int[][] newSolution = copySolution(solution);
			HousesState initialState = new HousesState(newSolution);
			initialState.getSolution()[NATIONALITY][ENGLISHMAN] = i;			
			calculateFeasibleDomains(initialState);
			solutionStack.push(initialState);
		}		
		while (!solutionStack.isEmpty()) {			
			HousesState currentState = solutionStack.peek();		
			...
			if (!currentState.getPossibleDomains().peek().getFeasibleValues().isEmpty()) {
				...
				HousesState newState = new HousesState(newSolution);
				newState.getSolution()[nextAttribute/5][nextAttribute%5] = nextValue;				
				calculateFeasibleDomains(newState);
				solutionStack.push(newState);
				numberOfStatesChecked++;
			} else {
				HousesState poppedState = solutionStack.pop();					
			}
		}
		return null;
	}
	...
	private static void calculateFeasibleDomains(HousesState state) {
		state.clearPossibleDomains();
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				if (state.getSolution()[i][j] == -1) {
					ArrayList<Integer> possibleValues = new ArrayList<Integer>();
					for (int k = 0; k < 5; k++) {						
						state.getSolution()[i][j] = k;
						if (consistencyCheck(state.getSolution())) {
							possibleValues.add(k);
						}
						state.getSolution()[i][j] = -1;
					}
					FeasibleDomains fd = new FeasibleDomains(i*5 + j, possibleValues);
	...
	// 1-5 Nationality, 6-10 Color, 11-15 Chocolates, 16-20 Drinks, 21-25 Animals
		...
		private static boolean consistencyCheck(int[][] solution) {
			...
			if (solution[COLOR][GREEN] != -1 && solution[COLOR][IVORY] != -1 
					&& Math.abs(solution[COLOR][GREEN] - solution[COLOR][IVORY]) != 1) {
